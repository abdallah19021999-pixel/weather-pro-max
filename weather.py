...
# --- محرك البحث العالمي (Smart Search) ---
@st.cache_data(ttl=600)
def fetch_global_weather(query):
    try:
        # هذا الرابط يبحث في قاعدة بيانات OpenWeather العالمية التي تشمل كل المدن والقرى
...
# --- محرك الجرافيكس الدائم (Visual Engine) ---
def apply_visuals(condition, temp):
    ...
    st.markdown(f"""
        ...
        .stButton button {{
            ...
            color: #000 !important; font-weight: bold !important; border-radius: 10px !important;
        }}
        ...
        <div class="weather-bg">{particles}</div>
...
if query:
    data = fetch_global_weather(query)
    if data:
        cond = data['weather'][0]['main'].lower()
        temp = data['main']['temp']
        apply_visuals(cond, temp)
        
        # ترجمة اسم المدينة المختارة
